...
long double evaluateComp_function1(int n, int i, int j, int p, int q, int *genes){
  if(genes[i]==p){
    if(genes[j]==q) return n-3; //alfa
    else return -2; //gamma
  }else if(genes[i]==q){
    if(genes[j]==p) return 1-n; //beta
    else return 0; //delta
  }else if(genes[j]==p){
    return 0; //delta
  }else if(genes[j]==q){
		    ...
		    for( j = 0; j < n; j++){
             ...
             if(abs(distAB) < 0.000001) continue;
			       for( p = 0; p < n; p++){
                        ...
                        fitness = fitness + evaluateComp_function1(n, i, j, p, q, genes) * distAB * zero_flow_matrix[p][q];
				           }
             }
		    }
	  }
...
long double evaluateComp_function2(int n, int i, int j, int p, int q, int *genes){
  if(genes[i]==p){
    if(genes[j]==q) return n-3; //alfa
    else return 0; //gamma
  }else if(genes[i]==q){
    if(genes[j]==p) return n-3; //beta
    else return 0; //delta
  }else if(genes[j]==p){
    return 0; //delta
  }else if(genes[j]==q){
    ...
    for( i = 0; i < n;i++){
        for( j = 0; j < n; j++){
             ...
             for( p = 0; p < n; p++){
                   for( q = 0; q < n; q++){
                        //if(p==q || flow_matrix[p][q] == 0) continue;
                        fitness = fitness + evaluateComp_function2(n, i, j, p, q, genes) * distAB * zero_flow_matrix[p][q];
                   }
             }
        }
    }
...
long double evaluateComp1_function3(int n, int i, int j, int p, int q, int *genes){
  if(genes[i]==p){
    if(genes[j]==q) return 2*n-3; //alfa
    else return n-2; //gamma
  }else if(genes[i]==q){
    if(genes[j]==p) return 1; //beta
    else return 0; //delta
  }else if(genes[j]==p){
    return 0; //delta
  }else if(genes[j]==q){
...
long double evaluateComp2_function3(int i, int p, int *genes){
	if(genes[i] == p) return 1;
	else return 0;
}
...
long double evaluateQAP_function3(int *genes){
    ...
    for( i = 0; i < n;i++){
        for( j = 0; j < n; j++){
             ...
             for( p = 0; p < n; p++){
                   for( q = 0; q < n; q++){
                        //if(p==q || flow_matrix[p][q] == 0) continue;
                        fitness = fitness + evaluateComp1_function3(n, i, j, p, q, genes) * distAB * zero_flow_matrix[p][q];
                   }
             }
        }
    }
    ...
    for (i=0;i<n;i++){
        ...
        for( p = 0; p < n; p++){
            ...
            fitness = fitness + evaluateComp2_function3(i, p, genes) * distAB * flow_matrix[p][p];
        }
    }
